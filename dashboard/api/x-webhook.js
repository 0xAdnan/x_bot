    ...
    try {
      ...
      if (body.tweet_create_events && Array.isArray(body.tweet_create_events)) {
        for (const tweet of body.tweet_create_events) {
          ...
          let targetUrl = urlMatch ? urlMatch[0] : "N/A";

          if (targetUrl !== "N/A" && !targetUrl.startsWith('http://') && !targetUrl.startsWith('https://')) {
          ...
          try {
            ...
            if (checkRes.ok) {
              ...
              if (existing && existing.length > 0) {
                console.log(`[X Webhook SKIP] Job already exists in Supabase for Tweet ID ${tweetId} (${tweetUrl}). Skipping duplicate.`);
                continue;
              }
            }
          } catch (err) {
          ...
          if (targetUrl === "N/A") {
            // Log mention without URL to Supabase so EVERY mention is tracked
            console.log(`[X Webhook] Tracking mention from ${userHandle} (No direct URL in text)`);
            ...
            }).catch(err => console.error("[X Webhook Sync Error]:", err));
            continue;
          }
