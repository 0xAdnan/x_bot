...
## THIS PROJECT: always Google Chrome, always the "Testing" profile

In this project opencode drives **real Google Chrome** and **only ever the "Testing" profile**
(Chrome profile directory `Profile 5`). Do not open other profiles; never default to the
generic `Default` profile.

- Bring the fleet up for Testing first: `awb up "Testing"` then confirm healthy via `awb status`
  (the `Testing` row, directory `Profile 5`).
- **Every** `/command` request must carry `"profile":"Testing"` — never omit it, never pass
  `"Default"` or another profile name. The router matches the profile by name (`Testing`) or
  directory (`Profile 5`).
- Any curl examples below that show `"profile":"Work"` (or omit `profile` entirely) are upstream
  illustrations; **in this project always substitute `"Testing"`**.
