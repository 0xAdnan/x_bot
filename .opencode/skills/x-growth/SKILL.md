...
description: >-
  ...
  This is the orchestration skill: use it to boot an X session with safety
  guardrails, run the session loop, manage the CRM state
  (prospects.jsonl / activity-log.jsonl / insights.md), enforce hard rules,
  and dispatch each flow to its dedicated skill (x-prospect, x-engage,
  x-outreach, x-content, x-community, x-mention). Put the flow details there,
  not here.
license: MIT
...
# x-growth, PITCH X/Twitter growth & sales (orchestrator)
...
The operation is split into one skill per flow. Load and follow the relevant
skill for each step instead of holding every playbook here:

| Flow | Skill | When |
|---|---|---|
| Boot, session loop, state, hard rules | **this skill (x-growth)** | Every session start |
| Prospecting / discovery / scoring | `x-prospect` | Finding & qualifying leads |
| Warm-up likes / replies / follows | `x-engage` | Advancing `new` → `warming`, engagement |
| DM outreach / sequences / objections | `x-outreach` | DM-ready prospects, conversions |
| Original posts / trends / memes / quotes | `x-content` | Any posting or content creation |
| Community help / invitations / orbit | `x-community` | Public help, threads, community building |
| Mention-demo bot (receipt → video → delivery) | `x-mention` | @trypitchdotco mentions, MCP jobs |

## Execution & autonomy

- **Act via real Chrome (agent-webbridge).** Drive the logged-in X session in
  Chrome's **"Testing" profile** through the `agent-webbridge` skill — POST to
  `http://127.0.0.1:10086/command` with `"profile":"Testing"` (navigate, click,
  fill, snapshot, screenshot, evaluate). First run `awb up "Testing"` and verify
  `awb status` shows `extensionConnected: true`. If the webbridge fleet is down,
  STOP and say so, never fake results.
...
Paths in the flow skills are given relative to the project root; the shared
files below live in `.opencode/skills/x-growth/`:
...
- `learn.md`, the learning loop: how to read outcomes and evolve which
  approaches you favor (tactics evolve; rules don't). Also aliased by the
  `x-growth` session retro step.
- `voice.md`, brand voice, approved claims, hard do-not list, compliance. Read
  before writing any outbound text; enforced from every flow skill.
- `state/account.json`, the operating identity: @adnanspitch, @trypitchdotco,
  ramp dates.
- `state/insights.md`, your adaptive memory for which openers/approaches win.
...
1. **Boot.** Read `safety.md` and `state/insights.md` (your adaptive memory, let
   ...
   - **Budget check:** `./target/release/pitch-cli budget`.
    ...
    - **Login + account check (before any action):** navigate to x.com/home and
      ...
      **@trypitchdotco**, not some other account (check the profile URL in the
      navigation bar). If logged OUT, perform re-login using the credentials
      saved in `.env` (`X_USERNAME`, `X_PASSWORD`) or
      `.opencode/skills/x-growth/state/account.json` (`username`, `password`).
      If re-login fails or hits a CAPTCHA, trip the circuit breaker and report.
      If a different account is logged in, STOP and report — never act on a wrong account.
...
3. **Trend scan** (if post/reply/quote budget allows) via `x-content` and
   `x-community`. Check @polymarket, X Explore/search, tech/startup accounts,
...
4. **Create content** (if quota allows) via `x-content`. Make original posts,
...
5. **Community scan** (if reply/quote budget allows) via `x-community`. Find
...
6. **Discover** (if quota allows) via `x-prospect`. Score, dedupe against
   `prospects.jsonl`, append new qualified ones at stage `new`.
7. **Engage** `new` → `warming` prospects via `x-engage`: like 1-2 recent
   posts, leave one genuinely useful reply. Never pitch in a first reply.
8. **Convert** DM-ready prospects via `x-outreach` (uses `dm-templates.md` in
   that skill's directory). Pick the opener variant `insights.md` favors for
   that segment (keep a ~10-20% exploration slot, see `learn.md`). Lead with
   value (offer/show a demo of *their* product), not a pitch. If they have
   already said something to you, respond to that message before adding the
   demo/product ask. Record the `variant` used on the prospect row and in the
   log.
...
10. **Follow up** within max-followups + opt-out rules (see `x-outreach`).
...
All `state/...` paths in this section resolve under
`.opencode/skills/x-growth/state/` (relative to the project root).
...
with `segment` + `variant` so the learning loop in `learn.md` can use them:
